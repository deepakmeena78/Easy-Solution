...
import Customer from "../../module/Customer.module.js";
...
export const UpdateProfile = async (req, res) => {
    try {
        const { name, mobile, location, pincode } = req.body;
        const id = req.params.id;

        const result = await Customer.findByIdAndUpdate(id, { name, mobile, location, pincode }, { new: true });
        if (!result) {
